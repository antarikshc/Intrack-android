...
class InvProvider : ContentProvider() {
     ...
     */
    private var mDbHelper: InvDbHelper? = null
    ...
    private fun insertItem(uri: Uri, values: ContentValues): Uri? {
        ...
        val name = values.getAsString(InvEntry.COLUMN_ITEM_NAME)
        ...
        val stock = values.getAsInteger(InvEntry.COLUMN_ITEM_STOCK)
        if (stock != null && stock < 0) {
            throw IllegalArgumentException("Inventory item requires valid stock amount.")
        }
        ...
        val id = database.insert(InvEntry.TABLE_NAME, null, values)
    ...
    override fun update(uri: Uri, contentValues: ContentValues?, selection: String?,
        ...
        return when (sUriMatcher.match(uri)) {
            INVENTS -> updatePet(uri, contentValues!!, selection, selectionArgs)
            ITEM_ID -> {
    ...
    private fun updatePet(uri: Uri, values: ContentValues, selection: String?, selectionArgs: Array<String>?): Int {
        ...
        val rowsUpdated = database.update(InvEntry.TABLE_NAME, values, selection, selectionArgs)
    ...
    override fun getType(uri: Uri): String? {
        return when (val match = sUriMatcher.match(uri)) {
            INVENTS -> InvEntry.CONTENT_LIST_TYPE
            ITEM_ID -> InvEntry.CONTENT_ITEM_TYPE
    ...
    companion object {
        ...
        val LOG_TAG: String = InvProvider::class.java.simpleName

        /**
         * URI matcher code for the content URI for the invents table
         */
        private const val INVENTS = 100
